...
{setvar(cintern("*slibu-version*"),
	cintern("$Id: slibu.c,v 1.5 2001/02/01 16:03:29 gjcarret Exp gjcarret $"),
	NIL);}
...
static LISP cgi_main(LISP flag,LISP result)
{int code;
 ...
 if (CONSP(result) && TYPEP(car(result),tc_string))
   {if ((code != 4) && (code != 6))
    ...
    if ((code != 5) && (code != 6))
      ...
      put_st("Content-type: text/xml\n\n");
    put_st("<?xml version=\"1.0\" encoding=\"ISO-8859-1\" ?>\n");
...
{int j,retval = 0,iargc,mainflag = 0,text_plain_flag = 0;
 char *iargv[2],*start,*end,htqs_arg_tmp[100];
 LISP l;
 iargv[0] = "";
 ...
 init_trace();
 init_slibu();
 init_subr_2("__cgi-main",cgi_main);
 if (iargc == 0)
   ...
   {for(j=1;j<(((mainflag >= 2) && (argc > 3)) ? 3 : argc);++j)
	...
	{sprintf(htqs_arg_tmp,"(__cgi-main %d (*catch 'errobj (main))))",
		 mainflag);
	 retval = htqs_arg(htqs_arg_tmp);}
      else
	retval = htqs_arg("(main)");}
