...
{setvar(cintern("*sql_oracle-version*"),
	cintern("$Id: sql_oracle.c,v 1.2 2000/04/26 14:43:39 gjcarret Exp gjcarret $"),
	NIL);}
    ...
    for(j = 1; j <= c->nselects; ++j)
	 ...
	 {colnam[colnamlen] = 0;
	  ...
	  if (siod_verbose_check(5))
	    {sprintf(dbuff,"column %d is oracle type %d, dbsize %d, dsize %d ",
		     j,c->selects[j-1].dbtype,
		     c->selects[j-1].dbsize,
		     c->selects[j-1].dsize);
	     put_st(dbuff);
	     put_st(colnam);
	  ...
	  switch(c->selects[j-1].dbtype)
	    {case ORACLE_ETYPE_INTEGER:
	     ...
	     case ORACLE_ETYPE_DATE:
	       ...
	       break;
	     case ORACLE_ETYPE_LONGRAW:
	       /* attempted conversion to string, error: ORA-00932 in oexec */
	       c->selects[j-1].etype = ORACLE_ETYPE_LONGRAW;
	       c->selects[j-1].buflen =  1000000;
	       c->selects[j-1].buf = (char *) malloc(c->selects[j-1].buflen+1);
	       c->selects[j-1].buf[0] = 0;
	       break;
	    case ORACLE_ETYPE_LONG:
	       /* This would just give silent truncation to string */
	       c->selects[j-1].etype = ORACLE_ETYPE_LONG;
	       c->selects[j-1].buflen =  1000000;
	       c->selects[j-1].buf = (char *) malloc(c->selects[j-1].buflen+1);
	       c->selects[j-1].buf[0] = 0;
	       break;
...
{long j;
 ...
 switch(sel->etype)
   {case ORACLE_ETYPE_FLOAT:
    ...
    default:
      if (siod_verbose_check(5))
	{sprintf(dbuff,"column %d, type %d unknown",j,sel->etype);
	 put_st(dbuff);
	 put_st("\n");}
      return(errswitch());}}
